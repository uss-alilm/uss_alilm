...
class ownership2_contract(models.Model):
    ...
    partner_id= fields.Many2one('res.partner','Customer',required=True, default=lambda self: self.env.user.partner_id,)
...
################################-------------------------------------------------------------############################################################################################################################################################################------------------------------------------------------
    building_invoice_ids = fields.One2many('account.move', string='Invoices', compute='_compute_building_invoices')    
    building_total_amount = fields.Float(string='Building Total Amount', compute='_compute_total_building_invoices_amount')
################################-------------------------------------------------------------############################################################################################################################################################################------------------------------------------------------
    @api.depends('building_invoice_ids') 
    def _compute_total_building_invoices_amount(self):
        for record in self:
            total = 0
            for invoice in record.building_invoice_ids:
                total += invoice.amount_total
            record.building_total_amount = total

    @api.depends('building')
    def _compute_building_invoices(self):
        for record in self:
            building = self.env['building'].search(['id','=',record.building.id])
            record.building_invoice_ids =   building.unit_building_total_amount
            # units = self.env['product.template'].search([('id', '=', record.building_unit.id)],limit=1)
            # for unit in units:
            #     invoices = self.env['account.move'].search([('invoice_line_ids.product_id', '=', record.building_unit.id)])
            #     record.building_invoice_ids = invoices
            #     break
################################-------------------------------------------------------------############################################################################################################################################################################------------------------------------------------------
    # @api.depends('building')
    # def _compute_building_invoices(self):
    #     for record in self:
    #         if record.building:
    #             building_units = record.building.unit_ids

    #             for unit in building_units:
    #                 invoices = self.env['account.move'].search([
    #                     ('invoice_line_ids.product_id', '=', unit.id),
    ...
    # def _compute_total_building_invoices_amount(self):
    #     for record in self:
    #         record.building_total_amount = sum(record.building_invoice_ids.mapped('amount_total'))
