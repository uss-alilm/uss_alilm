...
class rental_contract(models.Model):
    ...
    units = fields.Char(compute='_compute_units', store=True, string='All Units')            

    @api.depends('building')
    def _compute_units(self):
        for rec in self:
            building_id = rec.building.id  # Access related field for each record
            building_obj = self.env['product.template'].search([('building_id', '=', building_id)])
            if building_obj:
                i =len(building_obj)
                rec.units = len(building_obj)
            else:
                rec.units=i
    ...
    paints = fields.Selection([('accepted','مقبول'), ('Consists','تالف'), ], 'دهانات السقف و الجدران')
    grounds =  fields.Selection([('accepted','مقبول'), ('Consists','تالف'),  ], 'الارضيات')
    pathrooms = fields.Selection([('accepted','مقبول'),  ('Consists','تالف'), ], 'الحمامات و الاطقم الصحية')
    kitchen =  fields.Selection([('accepted','مقبول'), ('Consists','تالف'),  ], 'المطبخ')
    rooms =  fields.Selection([('accepted','مقبول'), ('Consists','تالف'), ], 'الغرف و الملاحق')
    electricity = fields.Selection([('accepted','مقبول'), ('Consists','تالف'), ], 'الكهرباء و الاضاءة')
    plumbing = fields.Selection([('accepted','مقبول'), ('Consists','تالف'), ], 'السباكة')
    water = fields.Selection([('accepted','مقبول'), ('Consists','تالف'),  ], 'خزان المياه')
    doors =fields.Selection([('accepted','مقبول'), ('Consists','تالف'), ], 'الأبواب')
    windows = fields.Selection([('accepted','مقبول'), ('Consists','تالف'),  ], 'الوافذ')
    air_conditions = fields.Selection([('accepted','مقبول'), ('Consists','تالف'), ], 'أجهزة التكييف')
    intercome = fields.Selection([('accepted','مقبول'), ('Consists','تالف'), ], 'الانتركوم')
    gibs = fields.Selection([('accepted','مقبول'), ('Consists','تالف'), ], 'أعمال الجبس')
    keys = fields.Selection([('accepted','مقبول'),('Consists','تالف'),], 'المفاتيح')
    grage = fields.Selection([('accepted','مقبول'), ('Consists','تالف'),], 'الكراج')
    others = fields.Selection([('accepted','مقبول'), ('Consists','تالف'), ], 'اخري')
...
###############################################################################################################################################
     ...
     #               rental_lines.append(
    #                    (0, 0, {'serial': ii, 'amount': self.insurance_fee, 'date': date_from, 'name': _('Insurance Deposit')}))
   #                 ii+= 1
  #              rental_lines.append((0, 0, {'serial': ii, 'amount': rental_fee, 'date': date_from, 'name': _('Rental Fee')}))
 #               ii+=1
#
 ...
 #                   rental_lines.append((0, 0, {'serial': ii, 'amount': rental_fee, 'date': new_date, 'name': _('Rental Fee')}))
#                    ii+=1
...
#################################################################################################################################################
            if rec.periodicity and rec.date_from and rec.date_to:
                ...
                date_from = rec.date_from
                date_to = rec.date_to
                rental_fee = rec.rental_fee/rec.n
                new_date = date_from
                if self.insurance_fee:
                    rental_lines.append(
...
class loan_line_rs_rent(models.Model):
    ...
    loan_id= fields.Many2one('rental.contract', '', readonly=True)#,ondelete='cascade'
    ...
    def send_multiple_installments_rent(self):
        ...
        template_id = ir_model_data.get_object_reference('itsys_real_estate', 'email_template_installment_notification_rent')[1]
...
class picking(models.Model):
    _name = "picking"
    _description = "أستمارات التسليم و التسلم"
    _inherit = ['mail.thread', 'mail.activity.mixin']

    rental_contract_id = fields.Many2one('rental.contract', required=True)
    attach_line= fields.One2many("rental.attachment.line", "contract_id_att", "Documents")
    reservation_id=  fields.Many2one('unit.reservation','Reservation')
